...
3. **Install and sign in to at least one AI CLI, and/or set an OpenRouter
   ...
   Then run the CLI for each subscription you have, once, to sign in
   through the browser tab it opens, then type `/quit`:
...
ships pre-built inside the zip.
...
- A status list — one row per provider (Claude Code, Codex, Gemini CLI,
  Qwen Code) — with a **●** (green, "detected") or **○** (grey, "not
  found") marker. "Detected" means Translate found that CLI (and, for
...
Clicking **Browse…** opens a popup listing every model in Translate's
catalog:
...
editor and reviewer model dropdowns in the top bar both show a model. If
they're blank, set defaults in Settings or pick from the dropdowns directly.
...
counter shows you it's still alive. The prompt timeout is 10 minutes per
call; if you hit that, it counts as transient, the agent is restarted, and
the call gets retried. The very first Claude or Codex round on a fresh
install can also be slow — up to 2 minutes — because `npx` is downloading
the ACP adapter package the first time; subsequent rounds are fast.
...
client/                    React + Vite + TypeScript SPA
  ...
  src/hooks/               useBook, useChapter, useEvents, useModels,
